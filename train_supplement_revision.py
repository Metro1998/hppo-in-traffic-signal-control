...
from src.rl.PPO_revision import *
from utils import *
from multiprocessing import Pool
from envs.ContinuousIntersection_v1 import ContinuousIntersectionEnv_v1
from envs.DiscreteIntersection_v1 import DiscreteIntersectionEnv_v1
from envs.HybridIntersection_v1 import HybridIntersectionEnv_v1
...
class Trainer(object):
    ...
    def unbatchify(self, value_action_logp: dict, agents_to_update: np.ndarray):
            ...
            with torch.no_grad():
                ...
                while True:
                    ...
                    if self.action_space_pattern == 'continuous':
                        ...
                        # action = action[ptr]
                        # [agents[k].buffer.store_con(observations[k], value_action_logp[k][1], reward[k], values[k], logp_actions[k], self.indicator[k])
                        #  for k, v in enumerate(info['agents_to_update'])]
                        ...
                        # [print(self.indicator[i])
                        #  for i in range(self.num_agent) if info['agents_to_update'][i] == 1]

                        # Automatically transit to next stage in the phase cycle if the agent has been executed.
                        # print('before', self.indicator)
                        # print('update', info['agents_to_update'])
                        self.indicator = (self.indicator + info['agents_to_update']) % self.num_stage
                        # print('after', self.indicator)
